...
{
    /// <summary>
    /// Сервис работы с файлами проекта (паттерн Repository).
    /// Отвечает за сохранение и загрузку документа в формате XML.
    /// </summary>
    public class Document_file_service : IDocument_service
    {
        private readonly IFigure_mapper figure_mapper;

        public Document_file_service(IFigure_mapper figure_mapper)
        {
            this.figure_mapper = figure_mapper;
        }
        ...
        public List<Figure_model> Load_document_from_file(string file_path)
        {
            ...
            using (FileStream file_stream = File.OpenRead(file_path))
            {
                ...
                // Восстанавливаем каждую фигуру через маппер (DI)
                foreach (Figure_record current_figure_record in document_record.figure_records)
                {
                    Figure_model figure_model =
                        figure_mapper.Create_model_from_record(current_figure_record);
